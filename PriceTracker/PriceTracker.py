...
def parse_price(raw: str) -> float | None:
    if not raw:
        return None
    cleaned = raw.replace('kr', '').replace('DKK', '').replace('\xa0', '').strip()
    ...
    for site in websites:
        ...
        try:
            ...
            if site['method'] == 'itemprop':
                ...
                if price_element:
                    raw_price = price_element.get('content', '').strip()
            ...
            if price is not None:
                payload = {
                    ...
                    "price": {
                        "price": price,
                        "retrievalDateTime": datetime.now(timezone.utc).isoformat()
                    }
                }
...
check_prices()
print("Scheduler running. Checks at 08:00 and 20:00 daily. Press Ctrl+C to stop.\n")
